...
/**
 * SuratPengaduanWordService
 *
 * Generates Surat Pengaduan Word documents using PhpWord TemplateProcessor.
 * Template variables sama persis dengan StpaWordService.
 */
class SuratPengaduanWordService
{
    public function generate(Laporan $laporan): string
    {
        $templatePath = app_path('Templates/surat_pengaduan_template.docx');

        if (!file_exists($templatePath)) {
            throw new \RuntimeException(
                'Template Surat Pengaduan tidak ditemukan di: ' . $templatePath .
                '. Silakan letakkan file surat_pengaduan_template.docx di app/Templates/'
            );
        }
        ...
        // Save to temp file
        $tempFile = tempnam(sys_get_temp_dir(), 'surat_pengaduan_') . '.docx';
    ...
    private function prepareTemplateData(Laporan $laporan): array
    {
        ...
        $hariLaporan = $parsedTanggalLaporan->translatedFormat('l');
        $tanggalLaporan = $parsedTanggalLaporan->translatedFormat('d F Y');
        $jamLaporan = $parsedTanggalLaporan->translatedFormat('H.i');

        // 2. FORMAT WAKTU KEJADIAN
        $waktuKejadian = $laporan->waktu_kejadian
            ? Carbon::parse($laporan->waktu_kejadian)->setTimezone('Asia/Jakarta')->translatedFormat('d F Y')
            : '-';
