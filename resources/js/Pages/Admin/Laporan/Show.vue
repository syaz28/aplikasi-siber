...
const selectedSubdit = ref(props.laporan?.assigned_subdit || '');

const assignSubdit = () => {
    if (!selectedSubdit.value) return;
    
    router.post(`/admin/laporan/${props.laporan.id}/assign`, {
...
<template>
    ...
    <AdminLayout :title="laporan.nomor_stpa || 'Detail Laporan'">
        <ToastContainer />
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="flex items-center justify-between mb-6">
                <div>
                    <Link href="/admin/laporan" class="inline-flex items-center gap-2 text-slate-600 hover:text-slate-800 transition mb-2">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div v-if="laporan.korban?.length > 0" class="space-y-4">
                            ...
                            <div class="mt-4 pt-4 border-t-2 border-slate-300 flex justify-between text-base font-bold">
                                ...
                                <span class="text-red-600">{{ formatRupiah(getTotalKerugian(laporan.korban)) }}</span>
                            </div>
                        </div>
                                            ...
                                            <div class="flex items-start gap-2">
                                                ...
                                                <div class="flex-1">
                                                    ...
                                                    <div class="text-xs text-slate-600 mt-1">
                                                        Perkara: <span class="font-mono text-amber-600">{{ match.nomor_stpa }}</span>
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div v-if="laporan.assigned_subdit" class="mb-4 p-3 bg-green-50 rounded-lg border border-green-200">
                            <p class="text-sm text-green-700">Sudah ditugaskan ke:</p>
                            <p class="font-semibold text-green-800 text-lg">Subdit {{ laporan.assigned_subdit }}</p>
                            <p class="text-sm text-green-600 mt-1">
                                oleh {{ laporan.assigned_by?.name || '-' }}<br/>
                                {{ formatDate(laporan.assigned_at) }}
                            </p>
                        </div>

                        <div class="space-y-3">
                            <label class="block text-sm font-medium text-slate-700">
                                {{ laporan.assigned_subdit ? 'Ubah Subdit' : 'Pilih Subdit' }}
                            </label>
                            <div class="grid grid-cols-3 gap-2">
                                <button
                                    ...
                                    :class="[
                                        'py-3 px-2 rounded-lg border-2 font-medium transition text-center',
                            ...
                            <button
                                @click="assignSubdit"
                                :disabled="!selectedSubdit || selectedSubdit === laporan.assigned_subdit"
                                class="w-full py-2.5 px-4 text-white bg-amber-500 rounded-lg hover:bg-amber-600 transition disabled:opacity-50 disabled:cursor-not-allowed font-medium"
                            >
                                {{ laporan.assigned_subdit ? 'Ubah Penugasan' : 'Tugaskan' }}
                            </button>
                        </div>
                    </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
                        ...
                        <div class="space-y-3">
                            ...
                            <div class="flex justify-between items-center py-2">
                                ...
                                <span class="font-semibold text-red-600">{{ formatRupiah(getTotalKerugian(laporan.korban)) }}</span>
                            </div>
                        </div>
                    </div>
